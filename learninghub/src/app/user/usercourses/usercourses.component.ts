...
userpayment = function(id: Number, technologyName: string, courseFee: Number, startDate: Date, endDate: Date) {
  ...
  this.http.get("https://localhost:44343/api/Admin/getuserid/"+localStorage.getItem('usermail')).subscribe(
    (result) => {
...
var body= JSON.stringify({
  ...
  this.http.post("https://localhost:44343/api/Admin/addtraining", body, 
...
getmentorprofile= function(id: Number) {
  this.http.get("https://localhost:44343/api/Admin/getmentorprofile/"+id).subscribe(
    (result) => {
      this.mentor = result;
      console.log(result)
      this.error_message = ""
